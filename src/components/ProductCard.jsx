...
function ProductCard({ product, size = "md", index = 0 }) {
  ...
  return (
    ...
    >
      ...
      <div className={`relative w-full overflow-hidden ${size === "sm" ? "aspect-[3/4]" : "aspect-[4/5]"} bg-white`}>
        {!failed ? (
        ...
        {/* Hover reveal */}
        <div className="pointer-events-none absolute inset-0 flex items-end justify-end bg-gradient-to-t from-ink/0 to-ink/0 p-4 opacity-0 transition-all duration-300 group-hover:from-ink/30 group-hover:opacity-100">
          <span className="flex h-11 w-11 items-center justify-center rounded-full bg-white/95 text-ink shadow-soft transition-transform duration-300 group-hover:scale-110">
            →
          </span>
        </div>
      </div>
      ...
      <div className="mt-5 flex items-start justify-between gap-4 border-b border-ink/10 pb-4">
        ...
        <p className={`${styles.priceSize} shrink-0 font-medium text-ink`}>
          ₹{product.price}
        </p>
      </div>
